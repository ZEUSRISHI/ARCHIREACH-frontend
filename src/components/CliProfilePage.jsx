      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
          <div className="flex items-center justify-between">
            <span
              className={`px-4 py-2 rounded-full text-sm font-medium ${contact.status === "new"
            ...
            <span className="text-sm text-gray-500">{new Date(contact.timestamp).toLocaleString()}</span>
          </div>
...
const NotificationSidebar = ({ isOpen, onClose, notifications, firmId, onViewContact, onMarkRead }) => {
...
export default function FProfilePage() {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/firm/${firmId}`)
      .then((res) => res.json())
      .then((data) => setFirmData(data))
      .catch((err) => console.error("Error fetching firm data:", err));

    fetch(`https://archireach.onrender.com/api/contacts?firmId=${firmId}`)
      .then((res) => res.json())
      .then((data) => {
        if (data.success) {
    ...
    setTimeout(() => {
      const mockNotification = {
        ...
        message:
          "I am interested in your architectural services for a commercial project. We are planning to build a 5-story office complex in Mumbai and would like to discuss design options.",
        firmId: firmId,
        firmName: "Your Firm",
        timestamp: new Date(),
        status: "new",
  ...
  const handleMarkRead = async (contactId) => {
    try {
      const response = await fetch(`https://archireach.onrender.com/api/contacts/${contactId}/status`, {
        method: "PATCH",
  ...
  const newContactCount = notifications.filter((n) => n.status === "new").length;
  ...
  return (
    <div className="bg-gray-50">
      ...
      <div className="w-full bg-gradient-to-r from-gray-900 to-gray-800 h-28 relative shadow-md">
        <div className="absolute top-12 left-28 flex items-end">
          ...
          <div className="ml-4 mb-2 text-gray-50">
            ...
            <div className="text-sm text-gray-300">Welcome to your firm dashboard</div>
          </div>
        </div>
      </div>

      {/* Buttons Row */}
      <div className="relative max-w-6xl mx-auto px-6 flex justify-end items-center -mt-10 pb-4 space-x-3 mt-2">
        <button
      ...
      {/* Modals */}
      <FPostAdd isOpen={isPostOpen} onClose={() => setPostOpen(false)} onSave={() => { }} />
